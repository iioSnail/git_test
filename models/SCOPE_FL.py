...
python c_train.py \
       --model SCOPE_FL \
       --datas wang271k,sighan13train,sighan14train,sighan15train,sighan13train,sighan14train,sighan15train \
       --bert-path FPT \
       --ckpt-path ./TrainedModels/scope.ckpt \
       --seed 2333 \
       --max-length 512 \
       --no-resume \
       ...
       --ckpt-path /TrainedModels/scope.ckpt \
       --finetune \
       --hyper-params weight_decay=0,lr=6e-5,warmup_proporation=0
```
...
class SCOPE_CSC_Model(pl.LightningModule):
    ...
    def predict(self, sentence):
        ...
        window = 0
        if 'window' in self.args.hyper_params:
            window = self.args.hyper_params['window']
...
class Dynamic_GlyceBertForMultiTask(BertPreTrainedModel):
    ...
    def forward(
        ...
        if labels is not None and pinyin_labels is not None:
            ...
            # 预测汉字的还使用FocalLoss
            masked_lm_loss = self.floss_fct(prediction_scores.view(-1, self.config.vocab_size), active_labels)
