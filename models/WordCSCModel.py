...
class WordCSCModel(pl.LightningModule):

    def __init__(self, args):
        super(WordCSCModel, self).__init__()
...
class ScopeWithPlugin(pl.LightningModule):
    ...
    def forward(self, inputs):
        # embeddings = self.model.model.bert.embeddings(input_ids=inputs['input_ids'],
        #                                               pinyin_ids=inputs['pinyin_ids'],
        #                                               token_type_ids=inputs['token_type_ids'])
        #
        # logits = self.plugin_model(embeddings, inputs['attention_mask'])
    ...
    def training_step(self, batch, batch_idx):
        ...
        """
        print(self.tokenizer.decode(inputs['input_ids'][0, 1:]))
        print(self.tokenizer.decode(logits.argmax(-1)[0, 1:]))
        print(self.tokenizer.decode(targets[0, 1:]))
        """
