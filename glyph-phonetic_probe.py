...
def load_model():
    ...
    model.load_state_dict(torch.load("./my_model/multimodal-sota.ckpt")['state_dict'])
    model = model.to("cuda")
...
class GlyphPhoneticModel(nn.Module):
    ...
    def build_inputs(self, chars):
        inputs = self.tokenizer(chars, return_tensors='pt')
        ...
        return inputs, input_pinyins, images
    ...
    def forward(self, inputs):
        ...
        return self.cls(torch.concat([a_features, b_features], dim=-1).squeeze(0)).view(-1)
