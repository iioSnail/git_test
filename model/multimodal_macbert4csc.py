...
class MacBert4CscModel(nn.Module):
    ...
    def forward(self, inputs, targets=None, detection_targets=None):
        ...
        logits = self.cls(bert_outputs.last_hidden_state)
        # 检错概率
        prob = self.detection(bert_outputs.last_hidden_state)

        if text_labels is None:
            # 检错输出，纠错输出
            outputs = (prob, logits)
        else:
            loss = self.loss_fct(logits.view(-1, len(self.tokenizer)), targets.input_ids.view(-1))
