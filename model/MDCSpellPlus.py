...
class CorrectionNetwork(nn.Module):
    ...
    def __init__(self, args):
        ...
        self.multi_modal_bert = MultiModalBertModel(args)
        self.tokenizer = self.multi_modal_bert.tokenizer
        self.bert = self.multi_modal_bert.bert
        ...
        self.dense_layer = nn.Linear(self.multi_modal_bert.hidden_size, len(self.tokenizer))
    ...
    def forward(self, inputs, word_embeddings, detect_hidden_states):
        ...
        """
        # 1. 使用bert进行前向传递
        bert_outputs = self.multi_modal_bert(input_ids=inputs['input_ids'],
                                             token_type_ids=inputs['token_type_ids'],
                                             attention_mask=inputs['attention_mask'],
                                             inputs_embeds=word_embeddings)
        # 2. 将bert的hidden_state和Detection Network的hidden state进行融合。
        bert_outputs['last_hidden_state'][:, :, :768] = bert_outputs['last_hidden_state'][:, :, :768] + detect_hidden_states
        # 3. 最终使用全连接层进行token预测
        return self.dense_layer(bert_outputs['last_hidden_state'])
...
class MDCSpellPlusModel(nn.Module):

    def __init__(self, args):
        super(MDCSpellPlusModel, self).__init__()
        self.args = args
