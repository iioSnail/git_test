...
model_name = st.selectbox("请选择您要使用的模型：", ["MultiModalBERT", "MacBert4CSC", "SCOPE"])
...
def load_model(model_name):
    if model_name == "MultiModalBERT":
        ...
        model_ = MyModel(mock_args(device=get_device(), hyper_params={}))
        ckpt_states = torch.load("./TrainedModels/multimodal-sota.ckpt", map_location='cpu')
        model_.load_state_dict(ckpt_states['state_dict'])
        model_ = model_.to(get_device())
        return model_
    ...
    if model_name == 'SCOPE':
        ...
        return SCOPE_CSC_Model.load_from_ckpt("./TrainedModels/SCOPE/FPT",
                                              "./TrainedModels/SCOPE/scope.ckpt", get_device())
