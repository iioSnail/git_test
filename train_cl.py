...
class Train(object):
    ...
    def __init__(self):
        ...
        self.model = DetectionCLModel().train().to(self.args.device)

        self.detection_optimizer = torch.optim.Adam(self.model.parameters(), lr=2e-5)
    ...
    def train_epoch(self):
        ...
        for i, (inputs, targets, d_targets) in enumerate(progress):
            ...
            d_loss = self.model.compute_loss(d_outputs, d_targets)
            d_loss.backward()
    ...
    def validate(self):
        ...
        for inputs, targets, d_targets in progress:
            inputs, targets, d_targets = inputs.to(self.args.device), \
                                                 targets.to(self.args.device), \
                                                 d_targets.to(self.args.device)
    ...
    def character_level_confusion_matrix(self, detection_outputs, detection_targets, mask):
